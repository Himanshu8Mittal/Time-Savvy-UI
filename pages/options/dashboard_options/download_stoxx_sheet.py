...
def download_stoxx_sheet(options: List[str]):
    ...
    # Download button for Stoxx Sheet
    if st.button("Download Stoxx Sheet"):
        data = {"project_code": selected_options, "month": month, "year": year}
        with st.spinner("Fetching stoxx sheets ... "):
            rr = requests.post(
                "http://127.0.0.1:8000/get_stoxx_timesheet/", json=data, timeout=1000
            )
            if rr.status_code == 200:
                b64 = base64.b64encode(rr.content).decode()
                href = f'<a href="data:application/zip;base64,{b64}" download="stoxx_sheets.zip">Download Stoxx Sheets</a>'
